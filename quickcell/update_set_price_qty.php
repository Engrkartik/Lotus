...
elseif($type=='view')
{
    ...
    while ($rows=mysqli_fetch_assoc($query1)) 
  {
     ...
     echo '<br><table class="table table-bordered" style="border:2px solid black;">';
  echo '<thead>
  <tr>
    <th>S.No</th>
    <th>COLOR</th>
  ...
  if($table=="myTable4")
  {
    ...
    if($qty<2){
    echo '</td><td>
      <input type="number" id="sum_qty'.$set_id.'" value="'.$qty.'" step="'.$qty.'" min="0" name="set_qty[]" onchange="sum(\'amt\','.$set_id.','.$qty.');save_price_table('.$rows['id'].',this.value,\'qty\');">   </td>
      <td><input id="sum_amt'.$set_id.'" type="number" value="'.$row2['wsp'].'" readonly=""></td>
    </tr>';
    }else{ echo '
    </td><td>
      <input type="number" id="sum_qty'.$set_id.'" value="'.$qty.'" step="'.$qty.'" min="0" name="set_qty[]" onchange="sum(\'amt\','.$set_id.','.$qty.');save_price_table('.$rows['id'].',this.value,\'qty\');" onkeydown="return false" >   </td>
      <td><input id="sum_amt'.$set_id.'" type="number" value="'.$row2['wsp'].'" readonly=""></td>
    </tr>';
    }
    echo '<tr>
  ...
  if($qty<2){
    echo '</td><td>
      <input type="number" id="sum_qty1'.$set_id.'" value="'.$qty.'" step="'.$qty.'" min="0" name="set_qty[]" readonly=""> </td>
      <td><input type="number" id="sum_amt'.$set_id.'" value="'.$row2['wsp'].'" onchange="sum(\'amt\','.$set_id.','.$qty.');save_price_table('.$rows['id'].',this.value,\'price\');"></td>

    </tr>';
    }else{ echo '
    </td><td><input type="number" id="sum_qty1'.$set_id.'" value="'.$qty.'" step="'.$qty.'" min="0" name="set_qty[]" readonly=""> </td>
      <td><input id="sum_amt'.$set_id.'" type="number" value="'.$row2['wsp'].'" onchange="sum(\'amt\','.$set_id.','.$qty.');save_price_table('.$rows['id'].',this.value,\'price\')"></td>
    </tr>';
    }
    echo '<tr>
  <td colspan="3">Total Set</td>
