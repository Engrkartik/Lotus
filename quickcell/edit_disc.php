...
$chk=mysqli_query($con,"SELECT * FROM `discount` WHERE discount.disc_id='$disc_name' and discount.aid='$admin_id'");
$rows=mysqli_fetch_assoc($chk);
$ddd=date('Y-m-d');
?>
 <div class="col-md-12">
            ...
            <div class="card card-warning">
                  ...
                  <div class="row">
                    <div class="col-sm-12">
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="disc_name" placeholder="Enter ..." value="<?=$rows['disc_name']?>" <?php if($rows['from_dt']<$ddd){echo "disabled";}?>>
                      </div>
                    </div>
                  </div>
                    <div class="row">
                    <div class="col-sm-6">
                      ...
                      <div class="form-group">
                        ...
                        <select class="form-control" id="disc_type" <?php if($rows['from_dt']<=$ddd){echo "disabled";}?>>
                          <option value="00" selected="selected" disabled="true">Select Discount Type</option>
                          <option <?php if($rows['disc_type']=='P'){echo "selected";}?> value="P">Percentage</option>
                          <option <?php if($rows['disc_type']=='A'){echo "selected";}?> value="A">Amount</option>
                        </select>
                      </div>
                    </div>
                  <!-- </div>
                  <div class="row"> -->
                    <div class="col-sm-6">
                      <div class="form-group">
                        ...
                        <input type="date" class="form-control " id="from_dt" value="<?=$rows['from_dt']?>" <?php if($rows['from_dt']<=$ddd){echo "disabled";}?> min='<?=$ddd?>' onchange="from_date()" >
                      </div>
                  ...
                  <div class="row">
                    <div class="col-sm-6">
                      <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="disc" placeholder="Enter ..." value="<?=$rows['disc']?>" <?php if($rows['from_dt']<$ddd){echo "disabled";}?> >
                      </div>
                    </div>
                    <div class="col-sm-6">
                      <!-- text input -->
                      <div class="form-group">
                       ...
                       <input type="date" id="to_dt" class="form-control" onchange="to_date()" value="<?=$rows['to_dt']?>" min="<?=$rows['from_dt']?>" >
                      </div>
                        <img src="images/cal.png" style="float: right;position: relative;margin-top: -40px;margin-left: 400px;">
              ...
              <div class="modal-dialog">
                ...
                <?php  $chk=mysqli_query($con,"SELECT * FROM category where (aid='$admin_id' or aid='0') and parent_id='0'");
  while($chk_r=mysqli_fetch_assoc($chk))
  {
  $cat_id=$chk_r['id'];
  // echo $admin;
  $query = mysqli_query($con,"SELECT product.* FROM `product` WHERE cat_id = '$cat_id' and aid='$admin_id' and status='A'");
