...
{
  $att_id=$_POST['att_id'];
$query=mysqli_query($con,"SELECT * FROM `prod_attribute` WHERE att_id='$att_id' and aid='$admin_id' Order by sub_cat asc");
echo '<div class="col-md-12" style="width:100%;"><div class="row" style="border:1px solid black;"><table class="table table-bordered table-responsive" style="border-collapse: collapse;">
 ...
 //     $n++;
      ...
      if(($prev==$row['sub_cat']))
      {
      echo "<td>".str_replace('_', ' ',$row['attribute'])."</td>";

      }else{
        $n++;
      echo "<tr><th>".$n."</th><th>".str_replace('_', ' ',$row['sub_cat'])."</th><td>".str_replace('_', ' ',$row['attribute'])."</td>";
      $prev=$row['sub_cat'];
...
for ($i=0; $i <$length ; $i++) { 
  ...
  else
  {
   ...
   if(sizeof($sum_avail_qty_arr)>0)
    ...
    if($insert_set)
    {
      $upd=mysqli_query($con,"UPDATE `product` SET `set_id`='$s_id',`auto_reduce_qty`='$auto_reduce_val' WHERE id='$pid'");
    }
    $sno++;
    $sno1++;
	}
                          ...
                          <!-- <div class="row"> -->
                            <div class="row">
                              <div class="col-md-12">
                                <div class="row">
                              <div class="col-md-2">
                              <button type="button" class="btn btn-warning" onclick="nexttab2(\'v-pills-colour_\''.$set_id.');color_tab('.$set_id.');show_size2(document.getElementById(\'cate_\''.$set_id.').value,'.$set_id.');">Next</button>
                              </div>
