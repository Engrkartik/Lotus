...
$chk=mysqli_query($con,"SELECT product.id,product.aid,product.title,product.item_name,product.cat_id,product.sale_price,product.discount,product.tax,if(product.avail_qty>0,product.avail_qty,'Out Of Stock') as avail_qty,product.img,product.feature,product.top,product.new,product.date,product.desc,product.brand,product.set_id,product.att_id,product.status,product.remark, add_cart.id AS idd,add_cart.cid,add_cart.pid,add_cart.size,add_cart.color,add_cart.qty,add_cart.price,add_cart.packing,prod_img.img_id,prod_img.img_url FROM `add_cart` LEFT JOIN product ON product.id=add_cart.pid LEFT JOIN prod_img ON prod_img.img_id=product.img WHERE add_cart.cid='$cust_id' AND product.aid='$admin_id' GROUP BY add_cart.pid");
