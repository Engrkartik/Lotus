...
if($_POST['type']=="add_w")
{
	...
	$avail_qty=0;
	$chkk=mysqli_query($con,"SELECT att_id FROM product WHERE id='$pid' and aid='$admin_id' order by att_id desc LIMIT 1");
	$c_row=mysqli_fetch_assoc($chkk);
	$att_id1=$c_row['att_id'];
	if($att_id==NULL)
	{
		$chkk1=mysqli_query($con,"SELECT att_id FROM product WHERE aid='$admin_id' order by att_id desc LIMIT 1");
	$c_row1=mysqli_fetch_assoc($chkk1);
	$att_id1=$c_row1['att_id'];
	$att_id=$att_id1+1;
		// $att_id=$att_id1+1;
	}else
	{
    ...
    if(mysqli_num_rows($chk)>0)
    {
		...
		if($count==0)
		{
			$chk3=mysqli_query($con,"SELECT * FROM `admin` WHERE id='$admin_id'");
			$row3=mysqli_fetch_assoc($chk3);
			if($row3['business_type']=='R')
			{
			$min_amt=mysqli_query($con,"SELECT MIN(wsp) as min,mrp,sum(qty_pcs) as qty_pcs FROM `set_details_whole` WHERE pid='$pid'");
			}else{
			$min_amt=mysqli_query($con,"SELECT MIN(wsp) as min,mrp,qty_pcs FROM `set_details_whole` WHERE pid='$pid'");
		...
		$insert=mysqli_query($con,"UPDATE `product` SET `title`='$title',`item_name`='$item',`cat_id`='$cat_id',`sub_cat`='$sub_cat',`sale_price`='$wsp',`mrp`='$mrp',`tax`='$gst',`gst_type`='$gst_type',`avail_qty`='$avail_qty',`img`='$img_id',`date`='$dj',`descrpt`='$dscrpt',`brand`='$brand',`auto_reduce_qty`='$auto_reduce_val' WHERE id='$pid' AND aid='$admin'");
	...
	if($insert)
	{
		...
		for ($i=0; $i <sizeof($att_array) ; $i++) { 
			...
			$att_id2=$att_idd[2];
			$sub_catt=mysqli_query($con,"SELECT A.id as Id,A.att_name AS Attribute,B.id as Sub_id,B.att_name As Sub_Cat,category.title as cat_name FROM `attributes` AS A,attributes AS B LEFT JOIN category ON category.id=B.cat_id WHERE A.att_id=B.id AND A.id='$att_id2' AND (category.aid='$admin' or category.aid='0')");
