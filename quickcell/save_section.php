...
if($type=="Top"){
	$pid = $_POST['new_id'];
	// echo $id;
	$avlbl = mysqli_query($con,"SELECT * FROM `product_order` WHERE pid = '$pid' and top != '0' and status ='A'");
	if((mysqli_num_rows($avlbl))<1){

	$findorder = mysqli_query($con,"SELECT * FROM `product_order` WHERE top !=0 order by id desc");
	$fetchorder = mysqli_fetch_array($findorder);
	$order = $fetchorder['top'];
	...
	if((mysqli_num_rows($findorder))>0){
		...
		// }
		// else{
			$inorder = mysqli_query($con,"INSERT INTO `product_order`(`pid`, `feature`, `top`, `new`, `date`) VALUES ('$pid','0','$val','0','$dj')");
		// }

		$upnew = mysqli_query($con,"UPDATE `product` SET `top`='Y' WHERE id='$pid'");
