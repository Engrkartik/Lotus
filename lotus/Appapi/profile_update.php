...
if(!empty($photo))
{
  $IDproofImage = $cust_id."_".uniqid()."_id.png";
    ...
    fclose($file1);
$query = mysqli_query($con,"UPDATE `company_reg` SET `ADDRESS`='$address',`EMAIL_ID`='$email',`image`='$path' WHERE aid='$admin_id' and id='$cust_id'");
   
     }
   else
{
$query = mysqli_query($con,"UPDATE `company_reg` SET `ADDRESS`='$address',`EMAIL_ID`='$email' WHERE aid='$admin_id' and id='$cust_id'");
