...
<div class="main_content_iner overly_inner ">
    <div class="container-fluid p-0 ">
        ...
        <!-- Page Title Ends -->
        <!-- Form Started -->
        <form action="{{ url('/update_product/'.request()->id) }}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
                        ...
                        <div class="row">
                            <div class="gallery">
                                @foreach ($img as $item=>$value)
                                    <img src="{{ 'http://34.72.9.224/quickWebsite/b2b_admin/'.$value->img_url }}" style="width: 100px;height:100px;margin:5px;border:2px solid grey;box-shadow:5px 5px 5px grey;">
                                @endforeach                                
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div class="row filter-row">
                                            <div class="col-6">
                                                <div class="form-group">
                                                    ...
                                                    <select name="category" id="category" class="form-control"
                                                        required="" onchange="gen_sub();">
                                                        <option value="">Select Category</option>
                                                        @foreach ($category as $item=>$value)
                                                            <option value="{{ $value->id }}" @if ($data->cat_id==$value->id)
                                                                selected="selected"
                                            ...
                                            <div class="col-6">
                                                <div class="form-group">
                                                    ...
                                                    <input class="form-control" type="text" placeholder="Product Name"
                                                        name="productname" id="productname" value="{{ $data->item_name }}">
                                                </div>
                                            </div>
                                            <div class="col-6">
                                                <div class="form-group">
                                            ...
                                            <div class="col-12">
                                                ...
                                                <div class="row">
                                                    <div class="col-6">
                                                        <label for=""
                                                            style="position: static;color: #797a7c;font-weight: 600;font-size: 16px;">SET</label>
                                                    </div>
                                                    <div class="col-6">
                                                        <a href="{{ url('/view_set/'.request()->id) }}" target="_blank" class="btn btn-primary add-button float-right"><i
                                                                class="fas fa-eye"></i>
                                                        </a>
                                                    </div>
                                                </div>
                                                    ...
                                                    <table class="table table-bordered">
                                                        ...
                                                        <tbody id="myTable">
                                                            @foreach ($attribute as $item=>$value)
                                                              ...
                                                              <tr class="att_{{ $item }}">
                                                                <td><input type="hidden" name="att[]" value="{{ $value->type }}">{{ $value->type }}</td>
                                                                <td><input type="hidden" name="sub_att[]" value="{{ $value->sub_type }}">{{ $value->sub_type }}</td>
                                                                <td>
                                                                    {{-- <a href="#" class="edit" title="Edit" data-toggle="tooltip"><i class="fas fa-edit"></i></a>&nbsp;&nbsp; --}}
                                                                   <a href="javascript:;" class="delete text-danger" title="Delete" data-toggle="tooltip"><i class="fas fa-trash" onclick="delete_att({{ $item }})"></i></a>
                                                                    </td>
                                                                </tr>
                                                                @php
                                                                    ++$item;
                                            ...
                                            <div class="col-12 mt-3">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control" placeholder="Product Description"
                                                        ...
                                                        id="productdesc">{{ $data->descrpt }}</textarea>
                                                </div>
                                            </div>
                                            <div class="col-12 mt-3">
...
function myColor() {
    var clr=JSON.parse(document.getElementById('color').value);
...
function mySize() {
    var szz=JSON.parse(document.getElementById('size').value);
    console.log(szz.length);
    for (let i = 0; i < szz.length; i++) {
        add_size(szz[i]);
    }
    if (szz.length>0) {
        save_size();
...
function gen_sub() {
    ...
    var main_cat=document.getElementById('category').value;
    ...
    $.ajax({
        type:'POST',
        url:'{{ url('gen_subcat') }}',
        data:{'cat':main_cat,'_token':token,'item':item},
        ...
        if (input.files) {
            ...
            for (i = 0; i < filesAmount; i++) {
                ...
                reader.onload = function(event) {
                    $($.parseHTML('<img>')).attr('src', event.target.result).attr('name','firstproductimg[]').css({"width":"100px", "height":"100px","margin":"5px","border":"2px solid grey","box-shadow":"5px 5px 5px grey"}).appendTo(placeToInsertImagePreview);
                }
...
function delete_size1(id) {
    console.log(id);
    var del_clr=document.getElementById('save_sz_div_'+id).value;
    $('.save_sz_div_'+id).remove();
    const index = size.indexOf(del_clr);
    if (index > -1) {
        size.splice(index, 1);
        }
