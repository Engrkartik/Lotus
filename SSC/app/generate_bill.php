...
$waiter_id = mysqli_real_escape_string($con,$_POST['wid']);
	...
	$query1 = mysqli_query($con,"SELECT * FROM `tables` WHERE id = '$table_no' ");

	if((mysqli_num_rows($query1))>0){

		$update = mysqli_query($con,"UPDATE `tables` SET `status`= 'empty' WHERE id = '$table_no'");
	}
	...
	while ($row=mysqli_fetch_assoc($query)) {
        ...
        $price = $row['price'];
		$html.='&ORDERDETAILS=[{%22WAITER%22:%22'.$waiter.'%22,%22TABLENO%22:%22'.$table_no.'%22,%22ITEM%22:%22'.$name.'%22,%22DESCRIPTION%22:%22%22,%22QTY%22:%22'.$quantity.'%22,%22UNIT%22:%22PCS%22,%22RATE%22:%222'.$price.'%22,%22AMOUNT%22:%22200%22,%22DATE%22:%22'.$date.'%22,%22STATUS%22:%22UNPAID%22,%22REMARKS%22:%22thanks%22}]';
    ...
    if($query){
		...
		// $response=file_get_contents('http://app.ssspltd.com/apiRestro/AddOrderDetails?DB=A100&MOB=8053520936&REMOTEID='.$id.'&ORDERNO='.$order_id);
	    // if($response)
		// {
			 $json = array(
	        "status" => true,
	...
	//         "status" => false,
	//         "details" => $data,
	//         "msg" => 'http://app.ssspltd.com/apiRestro/AddOrderDetails?DB=A100&MOB=8053520936&REMOTEID='.$id.'&ORDERNO='.$order_id.'&ORDERDETAILS=[{%22WAITER%22:%22'.$waiter.'%22,%22TABLENO%22:%22'.$table_no.'%22,%22ITEM%22:%22'.$name.'%22,%22DESCRIPTION%22:%22%22,%22QTY%22:%22'.$qty.'%22,%22UNIT%22:%22PCS%22,%22RATE%22:%222'.$price.'%22,%22AMOUNT%22:%22200%22,%22DATE%22:%22'.$date.'%22,%22STATUS%22:%22UNPAID%22,%22REMARKS%22:%22thanks%22}]'
	//     );
	// }
    }
    else{
